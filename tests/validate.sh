...
! rg -n "TODO|TBD|\\[TODO:" "$ROOT/README.md" "$ROOT/SKILL.md" "$ROOT/references" "$ROOT/examples" >/dev/null

grep -q "persona-autopsy-skill" "$ROOT/README.md"
grep -q "README_EN.md" "$ROOT/README.md"
grep -q "把语言怎样长成场面、价格、权力、欲望和成交拆开看" "$ROOT/README.md"
grep -q "拆的不是谁像谁。拆的是一句话怎样长出场面、价格、权力、欲望和成交" "$ROOT/README.md"
grep -q "145%不是数字，是先把全世界拖到他的报价单上" "$ROOT/README.md"
grep -q "杜蕾斯不靠露骨成交" "$ROOT/README.md"
grep -q "家里没矿别碰金融" "$ROOT/README.md"
grep -q "李佳琦卖的不是便宜" "$ROOT/README.md"
grep -q "快速短切样本" "$ROOT/README.md"
! rg -n "Nuwa|Nvwa|女娲" "$ROOT/README.md" >/dev/null

grep -q "Persona Autopsy" "$ROOT/README_EN.md"
grep -q "README.md" "$ROOT/README_EN.md"
grep -q "attention, power, desire, price, and conversion" "$ROOT/README_EN.md"
grep -q "Durex" "$ROOT/README_EN.md"
grep -q "Zhang Xuefeng" "$ROOT/README_EN.md"
grep -q "Li Jiaqi" "$ROOT/README_EN.md"

grep -q "拆一下这个人设" "$ROOT/SKILL.md"
grep -q "分析他的成交结构" "$ROOT/SKILL.md"
grep -q "Immersive Dialogue Mode" "$ROOT/SKILL.md"
grep -q "只做临时扮演" "$ROOT/SKILL.md"
grep -q "Weapon Library" "$ROOT/SKILL.md"
...
grep -q "145%不是数字" "$ROOT/examples/public-figure-autopsy.md"
...
grep -q "李佳琦" "$ROOT/examples/rapid-cuts.md"
grep -q "罗永浩" "$ROOT/examples/rapid-cuts.md"
